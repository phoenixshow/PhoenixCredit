...
//        super.onMeasure(widthMeasureSpec, heightMeasureSpec);
        ...
        for (int i = 0; i < childCount; i++) {
            ...
            //要想保证可以获取每个子视图的边距参数对象，必须重写generateLayoutParams()，如果不重写这里强转会出错
            ...
            if(lineWidth + childWidth + mp.leftMargin + mp.rightMargin <= widthSize){//不换行
                lineWidth += childWidth + mp.leftMargin + mp.rightMargin;//当前行宽累加上子视图的宽度及边距
                lineHeight = Math.max(lineHeight, childHeight + mp.topMargin + mp.bottomMargin);//当前行高与当前子视图高度取最大值
            }else {//换行
                width = Math.max(width, lineWidth);//已有宽度与最后行宽取最大值
                height += lineHeight;//行高累加最后行高度

                //重置
                lineWidth = childWidth + mp.leftMargin + mp.rightMargin;//最新行宽是当前子视图宽度
                lineHeight = childHeight + mp.topMargin + mp.bottomMargin;//最新行高是当前子视图高度
            }
        ...
        Log.e("TAG", "onMeasure--------->widthSize=" + widthSize + ",heightSize=" + heightSize + ",width=" + width + ",height=" + height);
    ...
    protected void onLayout(boolean changed, int l, int t, int r, int b) {
        ...
        Log.e("TAG", "onLayout--------->allViews.size=" + allViews.size() + ",allHeights.size=" + allHeights.size());
